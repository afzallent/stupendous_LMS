...
**User Story:** As an instructor, I want to create lessons with Markdown content, so that I can write rich text lessons without complex authoring tools.

#### Acceptance Criteria

1. WHEN an instructor creates a lesson THEN the LMS SHALL provide a Markdown editor with live preview
2. WHEN a student views a Markdown lesson THEN the LMS SHALL render the Markdown as formatted HTML
3. THE LMS SHALL support standard Markdown syntax including headings, lists, code blocks, links, and images
4. THE LMS SHALL support syntax highlighting for code blocks in common programming languages
5. WHEN a student completes viewing a Markdown lesson THEN the LMS SHALL generate an xAPI statement tracking the completion

### Requirement 12

**User Story:** As an instructor, I want to embed H5P interactive content in my courses, so that I can provide engaging interactive learning experiences.

#### Acceptance Criteria

1. WHEN an instructor uploads an H5P package THEN the LMS SHALL validate and store the package
2. WHEN a student interacts with H5P content THEN the LMS SHALL capture xAPI statements generated by the H5P player
3. THE LMS SHALL support common H5P content types including interactive videos, quizzes, presentations, and drag-and-drop activities
4. WHEN H5P content reports completion or scores THEN the LMS SHALL update the student's progress records
5. THE LMS SHALL provide an H5P content library where instructors can manage and reuse H5P packages

### Requirement 13

**User Story:** As an instructor, I want to embed external HTML content in lessons, so that I can integrate third-party interactive content and simulations.

#### Acceptance Criteria

1. WHEN an instructor configures an HTML embed lesson THEN the LMS SHALL allow specifying an external URL or inline HTML
2. WHEN a student views an HTML embed lesson THEN the LMS SHALL render the content in a sandboxed iframe
3. THE LMS SHALL support postMessage communication for tracking interactions from embedded content
4. IF embedded content sends xAPI statements via postMessage THEN the LMS SHALL validate and store them
5. THE LMS SHALL allow instructors to configure iframe sandbox permissions for security control

### Requirement 11

**User Story:** As an instructor, I want to create lessons with Markdown content, so that I can provide text-based learning materials with rich formatting.

#### Acceptance Criteria

1. WHEN an instructor creates a Markdown lesson THEN the LMS SHALL render the Markdown to HTML with proper formatting
2. THE LMS SHALL support standard Markdown syntax including headers, lists, code blocks, tables, and links
3. WHEN a student views a Markdown lesson THEN the LMS SHALL track scroll progress and reading time
4. WHEN a student completes reading a Markdown lesson THEN the LMS SHALL generate an xAPI statement with verb "completed"
5. THE LMS SHALL calculate and display estimated reading time based on word count

### Requirement 12

**User Story:** As an instructor, I want to embed H5P interactive content in lessons, so that students can engage with interactive learning activities.

#### Acceptance Criteria

1. WHEN an instructor adds H5P content THEN the LMS SHALL embed the content using a secure iframe
2. WHEN H5P content generates xAPI statements THEN the LMS SHALL capture and store them in the LRS
3. WHEN a student completes H5P content THEN the LMS SHALL update the student's progress records
4. THE LMS SHALL support H5P content from external H5P providers via embed URLs
5. WHERE H5P content includes scoring, the LMS SHALL capture and display the score in analytics

### Requirement 13

**User Story:** As an instructor, I want to embed PhET simulations in lessons, so that students can explore scientific concepts through interactive simulations.

#### Acceptance Criteria

1. WHEN an instructor adds a PhET simulation THEN the LMS SHALL embed the simulation with configurable dimensions
2. WHEN a student interacts with a PhET simulation THEN the LMS SHALL track interaction events
3. THE LMS SHALL support time-based or interaction-based completion criteria for PhET simulations
4. WHEN a student meets the completion criteria THEN the LMS SHALL generate an xAPI statement with verb "completed"
5. THE LMS SHALL display PhET simulation metadata including subject, grade level, and topics

### Requirement 14

**User Story:** As an instructor, I want to create lessons with custom HTML embeds, so that I can integrate various third-party interactive content.

#### Acceptance Criteria

1. WHEN an instructor adds an HTML embed THEN the LMS SHALL sanitize the HTML to prevent security vulnerabilities
2. THE LMS SHALL support iframe embeds with configurable sandbox attributes
3. WHEN a student views embedded content THEN the LMS SHALL track view events
4. THE LMS SHALL allow instructors to configure embed dimensions (width and height)
5. WHERE embedded content supports xAPI, the LMS SHALL capture and process xAPI statements

### Requirement 15

**User Story:** As a student, I want to see my progress across all lesson types, so that I can track my learning journey regardless of content format.

#### Acceptance Criteria

1. THE LMS SHALL track completion status for all lesson types (video, markdown, HTML embed, PhET, SCORM)
2. WHEN calculating course progress THEN the LMS SHALL include all lesson types in the calculation
3. THE LMS SHALL display lesson type icons to help students identify content types
4. WHEN a student views their progress THEN the LMS SHALL show time spent and interactions for each lesson type
5. THE LMS SHALL generate consistent xAPI statements across all lesson types for unified analytics
